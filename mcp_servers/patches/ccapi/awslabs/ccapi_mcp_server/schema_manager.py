...
class SchemaManager:
    ...
    def __init__(self):
        ...
        Uses /tmp/.ccapi_schemas so the cache is always writable.
        In AgentCore Runtime the package directory is read-only (extracted from
        ZIP into /var/task), so the original os.path.dirname(__file__)/.schemas
        path raises PermissionError at startup. /tmp is always writable.
        """
        cache_dir = os.environ.get('CCAPI_SCHEMA_CACHE_DIR', '/tmp/.ccapi_schemas')
